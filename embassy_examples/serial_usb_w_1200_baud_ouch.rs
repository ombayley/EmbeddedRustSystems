...
fn reboot_to_bootsel() -> ! {
    // These constants match rp235x-hal's reboot logic:
    // - BootSel kind = 0x0002
    // - Prefer ARM arch = 0x0010
    // - p1 flags: bit0 disables MSD, bit1 disables Picoboot (we want both enabled => 0)
    const REBOOT_KIND_BOOTSEL: u32 = 0x0002;
    const REBOOT_ARCH_ARM: u32 = 0x0010;

    let flags = REBOOT_KIND_BOOTSEL | REBOOT_ARCH_ARM;
    let delay_ms = 500;
    let p0 = 0;
    let p1 = 0; // keep MSD + Picoboot enabled

    let _ = unsafe { rom_data::reboot(flags, delay_ms, p0, p1) };

    // Should never return, but satisfy type checker.
    loop {
        cortex_m::asm::wfi();
    }
}
...
async fn cdc_task(mut class: CdcAcmClass<'static, MyUsbDriver>) -> ! {
    ...
    loop {
        ...
        // 🔴 1200-baud touch: check immediately after connection
        for _ in 0..50 {
